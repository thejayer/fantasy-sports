...
- Production hub (`SJ_SYNC_ON_START=0`) must not `stat`/`test` the GCS-mounted `SJ_DATA_DIR` before listen — a slow FUSE mount hangs the entrypoint and Cloud Run fails the port probe. GCS volume `mount-options` for gcloud must use **semicolons** (`uid=1001;gid=1001`); commas are parsed as separate volume keys.
- Hub CD defaults to **no GCS volume** (baked fixtures). Mounting `fantasy-sports-analytics-sj-data` is opt-in via **deploy hub → workflow_dispatch → bucket**, after the runtime SA can read the bucket (`./scripts/setup-sync-infra.sh`). Auto-mount on every push kept failing the Cloud Run port probe.
