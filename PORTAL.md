...
**People** lives at `/people` — a hand-edited directory of influential
accounts (Elon, Jensen Huang, AI lab leads, NFL/golf desk) with official
`x.com/{handle}` links. Edit `INFLUENTIAL_PEOPLE` in `apps/www/src/lib/people.ts`.
No X API.
